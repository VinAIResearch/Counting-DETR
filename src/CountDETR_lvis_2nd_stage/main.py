...
from data.fscd_lvis import FSCD_LVISDataset
...
def main(args):
    ...
    dataset_train = FSCD_LVISDataset(args,split='train')
    ...
    param_dicts = [
        {
            "params":
                [p for n, p in anchor_detr.named_parameters()
                    if not match_name_keywords(n, args.lr_backbone_names) and not match_name_keywords(n, args.lr_linear_proj_names) and p.requires_grad],
            "lr": args.lr,
        },
        {
            "params": [p for n, p in anchor_detr.named_parameters() if match_name_keywords(n, args.lr_backbone_names) and p.requires_grad],
            "lr": args.lr_backbone,
        },
        {
            "params": [p for n, p in anchor_detr.named_parameters() if match_name_keywords(n, args.lr_linear_proj_names) and p.requires_grad],
    ...
    if args.resume:
        ...
        pretrained_dict = {k: v for k, v in pretrained_model.items() if k in anchor_detr.state_dict() and "transformer.pattern." not in k}
    ...
    for epoch in range(args.start_epoch, args.epochs):
        train_stats = train_one_epoch(anchor_detr,criterion,dataloader_train,optimizer,args.device,epoch,args.clip_max_norm)
        lr_scheduler.step()
        if args.output_dir:
