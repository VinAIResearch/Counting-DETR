...
class Transformer(nn.Module):
    def __init__(
        ...
        self.modify_pattern = nn.Embedding(self.num_pattern, d_model)
    ...
    def forward(self, srcs, masks, points=None):
        ...
        if self.spatial_prior == "learned":
            ...
            assert points is not None, "provide points"
            self.num_position = points.shape[1]
            ref_points = points.squeeze(0)
            x = ref_points[:, 0]
            y = ref_points[:, 1]
            ...
            # self.num_position = points.shape[0] * points.shape[0]
            # x = torch.from_numpy(points[:, 0]).float()
            # y = torch.from_numpy(points[:, 1]).float()
            # xy = torch.meshgrid(x, y)
            # reference_points=torch.cat([xy[0].reshape(-1)[...,None],xy[1].reshape(-1)[...,None]],-1).cuda()
        ...
        tgt = (
            self.modify_pattern.weight.reshape(1, self.num_pattern, 1, c)
        ...
        outputs_cls = torch.cat(outputs_classes, dim=0)
        outputs_xywh = torch.cat(outputs_coords, dim=0)
        outputs_xy = outputs_xywh[..., :2]
        outputs_wh = outputs_xywh[..., 2:]

        return outputs_cls, outputs_xy, outputs_wh
