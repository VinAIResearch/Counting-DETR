...
from engine import evaluate, train_one_epoch, generate_pseudo_label, test
...
def get_args_parser():
    ...
    # dataset parameters
    parser.add_argument('--dataset_file', default='fscd_147', choices=['fscd_147', 'fscd_lvis', 'fscd_147_point', "fscd_147_test"])
...
def main(args):
    ...
    if args.resume and not args.generate_pseudo_label:
        ...
        filtered_pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict and k != "transformer.pattern.weight"}
    ...
    if args.generate_pseudo_label:
        ...
        generate_pseudo_label(model, criterion, postprocessors,
                                data_loader_train, "train", device, args.output_dir, args.vis_pseudo)
        generate_pseudo_label(model, criterion, postprocessors,
                                data_loader_val, "val", device, args.output_dir, args.vis_pseudo)
        dataset_test = build_dataset(image_set='test', args=args)
        data_loader_test = DataLoader(dataset_test, batch_size=args.batch_size, shuffle=True)
        generate_pseudo_label(model, criterion, postprocessors,
                                data_loader_test, "test", device, args.output_dir, args.vis_pseudo)
    ...
    if args.test:
        ...
        test(model, criterion, postprocessors,
                                            data_loader_test, device, args.output_dir, args.vis_pseudo)
