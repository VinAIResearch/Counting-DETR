...
def infer(model, criterion, data_loader, device, output_dir, split="test"):
    output_path = os.path.join(output_dir, "predictions_" + split + ".json")
    if os.path.isfile(output_path):
        os.remove(output_path)
        print("remove_", output_path)
    model.eval()
    criterion.eval()
    ...
    for ret in metric_logger.log_every(data_loader, 10000, header):
        ...
        targets = [{"boxes": ret["boxes"].squeeze(0).to(device), "labels": ret["labels"].squeeze(0).to(device)}]
...
def main(args):
    ...
    dataset_val = build_test_dataset(args=args, image_set="val")
    data_loader_val = DataLoader(dataset_val, batch_size=args.batch_size, shuffle=False)

    dataset_test = build_test_dataset(args=args, image_set="test")
    ...
    infer(model, criterion, data_loader_val, device, args.output_dir, split="val")

    infer(model, criterion, data_loader_test, device, args.output_dir, split="test")
