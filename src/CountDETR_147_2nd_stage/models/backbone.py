...
class BackboneAgg(nn.Module):
    ...
    def extract_feature(self, tensor_list: NestedTensor, rects):
        xs = self.body(tensor_list.tensors)
        aggregate_dict = dict()
        for name, x in xs.items():
        ...
        for name, x in xs.items():
            m = tensor_list.mask
            aggregated_features = aggregate_dict[name]
            assert m is not None
            mask = F.interpolate(m[None].float(), size=x.shape[-2:]).to(torch.bool)[0]
...
def build_backbone(args):
    ...
    # backbone = OldBackbone(args.backbone, train_backbone, return_interm_layers, args.dilation)
