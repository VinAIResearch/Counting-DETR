...
class Transformer(nn.Module):
    def __init__(
        ...
        decoder_layer = TransformerDecoderLayer(
            d_model, dim_feedforward, dropout, activation, nhead, num_feature_levels, attention_type
        )
        ...
        self.adapt_pos2d = nn.Sequential(nn.Linear(d_model, d_model), nn.ReLU(), nn.Linear(d_model, d_model),)
        self.adapt_pos1d = nn.Sequential(nn.Linear(d_model, d_model), nn.ReLU(), nn.Linear(d_model, d_model),)
        ...
        self.bbox_variance = MLP(d_model, d_model, 2, 3)
    ...
    def _reset_parameters(self):
        ...
        nn.init.constant_(self.bbox_variance.layers[-1].weight.data, 0.01)
        nn.init.constant_(self.bbox_variance.layers[-1].bias.data, 0.01)
        ...
        self.bbox_variance = nn.ModuleList([self.bbox_variance for _ in range(num_pred)])

    def forward(self, srcs, masks, points=None):
        ...
        elif self.spatial_prior == "defined":
            assert points is not None, "defined, provide points"
            self.num_position = points.shape[0]
            x = torch.from_numpy(points[:, 0]).float()
            y = torch.from_numpy(points[:, 1]).float()
            reference_points = torch.stack([x, y]).cuda().permute(1, 0)
            reference_points = reference_points.unsqueeze(0).repeat(bs, self.num_pattern, 1)
        elif self.spatial_prior == "sampled":
            assert points is not None, "sampled, provide points"
            reference_points = points.repeat(bs, self.num_pattern, 1)
        else:
        ...
        for lid, layer in enumerate(self.decoder_layers):
            ...
            outputs_class = self.cls_embed[lid](output)
            ...
            outputs_var = self.bbox_variance[lid](output)
        ...
        output = torch.cat(outputs_classes, dim=0), torch.cat(outputs_coords, dim=0), torch.cat(outputs_vars, dim=0)

        return output, reference_points


class TransformerEncoderLayerSpatial(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024, dropout=0.0, activation="relu", n_heads=8, attention_type="RCDA"):
        super().__init__()
...
class TransformerEncoderLayerLevel(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024, dropout=0.0, activation="relu", n_heads=8):
        super().__init__()
    ...
    def forward(self, src, level_emb=0):
        ...
        src2 = self.self_attn_level(
            src.reshape(bz, h * w, c) + level_emb, src.reshape(bz, h * w, c) + level_emb, src.reshape(bz, h * w, c)
        )[0].reshape(bz, h, w, c)
...
class TransformerDecoderLayer(nn.Module):
    def __init__(
        self, d_model=256, d_ffn=1024, dropout=0.0, activation="relu", n_heads=8, n_levels=3, attention_type="RCDA"
    ):
        super().__init__()
